...
export function PortalCockpitLayout({
  ...
  const offer = incomingOffers.length > 0 ? incomingOffers[0] : null;
  const job = activeAssignedJob || (activeJobs.length > 0 ? activeJobs[0] : null) || offer;
  const status = (job?.status || offer?.status || (offer ? 'OFFERED' : 'STANDBY')).toUpperCase();

  const isOffer = Boolean(offer && (!activeAssignedJob || activeJobs.length === 0));
  ...
  // Verification Gate Statuses (All 4 are strictly mandatory)
  ...
  const customerPhone = job?.customer_phone || offer?.customer_phone;
  const payoutAmount = job?.estimated_payout || job?.estimated_price || job?.price || offer?.estimated_payout || offer?.price || 550;
  const distanceKm = job?.distance_km || offer?.distance_km || '0.0';
  ...
  return (
    <div className="w-full h-full flex-1 flex flex-col font-sans bg-slate-100/90 text-slate-900 overflow-hidden p-3 sm:p-4 lg:p-4">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 min-h-0 bg-white rounded-2xl border border-slate-200/90 shadow-card overflow-hidden">
        {/* ── LEFT COLUMN: FIRST-PERSON DRIVING NAVIGATION / SITE MAP (7 Cols / 58%) ── */}
        <div className="lg:col-span-7 h-full w-full relative min-h-[420px] lg:min-h-full flex flex-col bg-slate-950 border-b lg:border-b-0 lg:border-r border-slate-200 overflow-hidden">
          {/* Interactive First-Person Live Navigation Road Map */}
        ...
        {/* ── RIGHT COLUMN: JOB CARD, STEPPER & PRE-SERVICE VERIFICATION (5 Cols / 42%) ── */}
        <div className="lg:col-span-5 h-full bg-white flex flex-col justify-between p-5 sm:p-6 overflow-y-auto space-y-4">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="px-2 py-0.5 rounded text-[10px] font-mono font-black uppercase bg-emerald-100 text-emerald-800 border border-emerald-300">
                  {isOffer ? 'NEW JOB OFFER' : isArrived ? 'SITE ARRIVAL' : isEnRoute ? 'EN ROUTE' : isInProgress ? 'IN PROGRESS' : 'ACTIVE ASSIGNMENT'}
                </span>
                <span className="font-mono text-xs font-bold text-slate-400">
                  SR-{job?.request_id || job?.id || '4950'}
                </span>
              </div>

              <div>
                <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight leading-tight">
                  {job?.service_title || job?.service_category || 'USB Charging Socket Fit'}
                </h1>
                <p className="text-xs text-slate-500 font-medium mt-0.5">
                  {job?.service_description || 'Deep Chemical Tub Wash & Sanitization'}
                </p>
              </div>
            ...
            {/* ── HORIZONTAL STEPPER LINE (Accepted ── En Route ── Arrived) ── */}
            ...
            {/* ── PRE-SERVICE VERIFICATION SECTION (All 4 Fields Required, No Skip) ── */}
              ...
              <div className={`p-3.5 rounded-xl border space-y-2.5 transition-all ${
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (openLiveCamera) {
                        openLiveCamera('Capture Presence Selfie', 'user', 'presence_selfie', (file) => handlePhotoUploadSubmit('presence', file));
                      }
                    }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (openLiveCamera) {
                        openLiveCamera('Capture Work Area Photo', 'environment', 'pre_work_area', (file) => handlePhotoUploadSubmit('work_area', file));
                      }
                    }}
                    className={`border-2 border-dashed rounded-xl p-3.5 flex flex-col items-center justify-center gap-1.5 text-center transition-all cursor-pointer ${
                      isWorkAreaPhotoDone
          ...
          {/* ── BOTTOM ACTION BUTTON: Start Service Execution (Gated on all 4 fields) ── */}
