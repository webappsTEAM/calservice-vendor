...
def apply_transition(service_request, target_status: str, actor=None) -> str:
    ...
    if target == "accepted":
        try:
            ...
            cat_clean = (service_request.service_category or "").strip().lower()
            if cat_clean in LOGISTICS_SERVICE_CATEGORIES:
                initial_leg = "ASSIGNED" if cat_clean == "packers_movers" else "EN_ROUTE_PICKUP"
