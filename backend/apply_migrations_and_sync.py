    ...
    try:
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS workforce_rate_card (
                ...
                section VARCHAR(50) NOT NULL DEFAULT 'MATERIAL',
                ...
                max_discount_percent NUMERIC(5, 2) NOT NULL DEFAULT 20.00,
                is_active BOOLEAN NOT NULL DEFAULT TRUE,
                sort_order INTEGER NOT NULL DEFAULT 0,
                created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
            );
            CREATE INDEX IF NOT EXISTS idx_wfrc_cat_sec ON workforce_rate_card(service_category, section);

            ALTER TABLE workforce_quote
                ADD COLUMN IF NOT EXISTS quote_number VARCHAR(50),
                ADD COLUMN IF NOT EXISTS quote_version INTEGER DEFAULT 1,
                ADD COLUMN IF NOT EXISTS work_job_id BIGINT,
                ADD COLUMN IF NOT EXISTS service_category VARCHAR(150) DEFAULT '',
                ADD COLUMN IF NOT EXISTS service_name VARCHAR(200) DEFAULT '',
                ADD COLUMN IF NOT EXISTS inspection_fee NUMERIC(10, 2) DEFAULT 0.00,
                ADD COLUMN IF NOT EXISTS inspection_fee_adjusted NUMERIC(10, 2) DEFAULT 0.00,
                ADD COLUMN IF NOT EXISTS net_payable NUMERIC(10, 2) DEFAULT 0.00,
                ADD COLUMN IF NOT EXISTS decision_token VARCHAR(64),
                ADD COLUMN IF NOT EXISTS decision_expires_at TIMESTAMP WITH TIME ZONE,
                ADD COLUMN IF NOT EXISTS customer_decision VARCHAR(30) DEFAULT '',
                ADD COLUMN IF NOT EXISTS customer_decided_at TIMESTAMP WITH TIME ZONE,
                ADD COLUMN IF NOT EXISTS customer_decline_reason TEXT DEFAULT '',
                ADD COLUMN IF NOT EXISTS customer_notes TEXT DEFAULT '',
                ADD COLUMN IF NOT EXISTS structural_impact VARCHAR(30) DEFAULT 'NONE',
                ADD COLUMN IF NOT EXISTS admin_cleared_by_id BIGINT,
                ADD COLUMN IF NOT EXISTS admin_cleared_at TIMESTAMP WITH TIME ZONE,
                ADD COLUMN IF NOT EXISTS admin_clearance_notes TEXT DEFAULT '',
                ADD COLUMN IF NOT EXISTS sent_at TIMESTAMP WITH TIME ZONE;

            CREATE INDEX IF NOT EXISTS idx_wfq_job_status ON workforce_quote(job_id, status);
            CREATE INDEX IF NOT EXISTS idx_wfq_dec_token ON workforce_quote(decision_token);

            ALTER TABLE workforce_quote_item
                ADD COLUMN IF NOT EXISTS section VARCHAR(50) DEFAULT 'OTHER',
            ...
            ALTER TABLE workforce_painting_quote
                ADD COLUMN IF NOT EXISTS property_type VARCHAR(100) DEFAULT 'Apartment',
                ADD COLUMN IF NOT EXISTS rooms_detail JSONB DEFAULT '[]'::jsonb,
                ADD COLUMN IF NOT EXISTS surface_condition VARCHAR(100) DEFAULT 'Good',
                ADD COLUMN IF NOT EXISTS existing_paint_condition VARCHAR(100) DEFAULT 'Old Emulsion',
                ADD COLUMN IF NOT EXISTS brand_grade VARCHAR(100) DEFAULT 'Asian Paints / Berger',
                ADD COLUMN IF NOT EXISTS crack_treatment BOOLEAN DEFAULT FALSE,
                ADD COLUMN IF NOT EXISTS waterproofing_needed BOOLEAN DEFAULT FALSE,
                ADD COLUMN IF NOT EXISTS scaffolding_required BOOLEAN DEFAULT FALSE,
                ADD COLUMN IF NOT EXISTS notes TEXT DEFAULT '';

            ALTER TABLE workforce_mason_quote
                ...
                ADD COLUMN IF NOT EXISTS area_sqft NUMERIC(10, 2) DEFAULT 0.00,
                ADD COLUMN IF NOT EXISTS structural_impact VARCHAR(50) DEFAULT 'NONE',
                ADD COLUMN IF NOT EXISTS access_difficulty VARCHAR(50) DEFAULT 'Standard',
                ADD COLUMN IF NOT EXISTS labour_count INTEGER DEFAULT 2,
                ADD COLUMN IF NOT EXISTS notes TEXT DEFAULT '';
        """)
        print("WorkforceQuote and rate card tables & columns verified in PostgreSQL.")
    except Exception as e:
        print("WorkforceQuote schema sync notice:", e)
